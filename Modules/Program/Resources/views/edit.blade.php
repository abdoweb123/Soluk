...
@section('content')
<form method="POST" action="{{ route(activeGuard().'.programs.update',$Model) }}" enctype="multipart/form-data" >
    @csrf
    @method('PUT')
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-sm-12">
            ...
            <textarea name="desc_ar" placeholder="@lang('trans.desc_ar')" class="form-control mceNoEditor" required>{!! $Model['desc_ar'] !!}</textarea>
        </div>
        <div class="col-md-6 col-sm-12">
            <label >@lang('trans.desc_en')</label>
            <textarea name="desc_en" placeholder="@lang('trans.desc_en')" class="form-control mceNoEditor" required>{!! $Model['desc_en'] !!}</textarea>
        </div>
